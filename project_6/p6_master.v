...
`timescale 1 ns / 1 ps
    ...
    localparam integer MAX_X_POS = (GROUP_WIDTH - 1) - FACE_BUFFER_WIDTH + 1;
    localparam integer MAX_Y_POS = (GROUP_HEIGHT - 1) - FACE_BUFFER_HEIGHT + 1;
...
//Forward movement occurs when the write channel is valid and ready
        ...
        if (M_AXI_ARESETN == 0 || init_txn_pulse == 1'b1)
            ...
            case (gState)
                ...
                gCOMPUTE: begin
                    ...
                    axi_araddr <= (axi_araddr >= ( (GROUP_HEIGHT - 1) * GROUP_WIDTH + group_src_addr) ) ? group_src_addr + xIndexNow + 32'd1 : axi_araddr + GROUP_WIDTH;
                    //large or equla than is right, not just equal since there will be only one match and when traversing to the bottom of the second column, the address can't jump back to the top of the third column;
                end
                default: begin
    ...
    */
    ...
    // always block for innerShiftDataSignal signal
    always @ (posedge M_AXI_ACLK) begin
        if(startAdderSignal)
            innerShiftDataSignal <= 1'b1;
        else if(numberOfRowsNotTransfered && (cCounter == NEW_ROWS_BUFFER_HEIGHT - 1))
            innerShiftDataSignal <= 1'b0;
        else if(numberOfRowsNotTransfered == 12'd0 && (cCounter == (GROUP_HEIGHT & 4'd15) - 1))
            innerShiftDataSignal <= 1'b0;
        else
