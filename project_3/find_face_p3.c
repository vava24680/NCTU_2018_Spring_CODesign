...
{
	int loopIndex;
	/*uint8 *zero_array = malloc(sizeof(uint8) * 1024);
	...
	{
		*regBankRegister = (loopIndex + 32);
		memmove((void *)interfaceRegisters, face + loopIndex * FACE_WIDTH, 32);
		//memmove((void *)interfaceRegisters, zero_array + loopIndex * FACE_WIDTH, sizeof(uint8) * 32);
		//memmove((void *)interfaceRegisters, non_zero_array + loopIndex * FACE_WIDTH,  sizeof(uint8) * 32);
	}
}
	...
	if(row == 0)
	{
		for(loopIndex = 0; loopIndex < FACE_HEIGHT; loopIndex++)
		{
			...
			*regBankRegister = loopIndex;
			memmove((void *)interfaceRegisters, group + loopIndex * width + col, 32);
			//memmove((void *)interfaceRegisters, non_zero_array + loopIndex * FACE_WIDTH, sizeof(uint8) * 32);
			//memmove((void *)interfaceRegisters, zero_array + loopIndex * FACE_WIDTH, sizeof(uint8) * 32);
		}
	}
